...
class TimerListener(QTimer):
	def __init__(self, cb=None, **kwargs):
		super(TimerListener, self).__init__(**kwargs)
		self.categories = set([object()])
		self.cb = cb
...
class CronListener(QTimer):
	def __init__(self, cb=None, **kwargs):
		super(CronListener, self).__init__(**kwargs)
		self.categories = set([object()])
		self.cb = cb
...
class CronTests(unittest.TestCase):
	def test_next(self):
		self.assertEquals(datetime(1970, 1, 1, 0, 0, 1), nextTime(datetime(1970, 1, 1)))
		self.assertEquals(datetime(1970, 1, 1, 0, 1, 0), nextTime(datetime(1970, 1, 1), minute=[1]))
		self.assertEquals(datetime(1970, 1, 1, 2, 0, 0), nextTime(datetime(1970, 1, 1), hour=[2, 4]))

		self.assertEquals(datetime(1970, 1, 1, 2, 1, 1), nextTime(datetime(1970, 1, 1, 2, 1, 0), hour=[2, 4]))
		self.assertEquals(datetime(1970, 1, 1, 4, 0, 0), nextTime(datetime(1970, 1, 1, 3, 1, 0), hour=[2, 4]))
		self.assertEquals(datetime(1970, 1, 1, 4, 0, 0), nextTime(datetime(1970, 1, 1, 2, 1, 0), hour=[2, 4], minute=[0]))

		self.assertEquals(datetime(1971, 2, 10), nextTime(datetime(1970, 2, 28), month=[2], day=[10, 20]))

		self.assertEquals(datetime(1972, 2, 29), nextTime(datetime(1971, 1, 1), month=[2], day=[29]))
